...
import { contentTokenDisplay, isFormLearned } from '../src/lib/learned'
...
function learn(progress: Progress, idx: number): Progress {
  return { ...progress, units: { ...progress.units, [idx]: { lvl: 1 } } }
}
...
async function main() {
  ...
  const idxOf = (c: string) => index.byForm.get(c)!.idx

  const display = (tok: WordToken, p: Progress) =>
    contentTokenDisplay(tok, (c) => isFormLearned(c, index, p), index.lang)
  ...
  p = learn(p, idxOf('私'))
  checks.push(['私 japanese once 私 learned', display(watashi, p) === 'native'])
  checks.push(['学生 still english (学/生 unlearned)', display(gakusei, p) === 'english'])

  p = learn(p, idxOf('学')) // learn only ONE of the two kanji
  checks.push(['学生 japanese once 学 learned (≥1 rule)', display(gakusei, p) === 'native'])

  // No tracked kanji: kana-only words stay Japanese; words with untracked kanji show English.
  const aru: WordToken = { kind: 'word', surface: 'ある', reading: 'ある', gloss: 'is', units: [], targets: [] }
  const shima: WordToken = { kind: 'word', surface: '島', reading: 'しま', gloss: 'island', units: [], targets: [] }
  checks.push(['kana-only word (ある) is japanese', display(aru, p) === 'native'])
