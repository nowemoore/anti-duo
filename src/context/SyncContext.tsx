...
import { getLocalRev, setLocalRev } from '../lib/rev'
 ...
 *
 * Signed out it does nothing at all, which is the normal state — an account is opt-in.
 */
    ...
    ;(async () => {
      try {
        ...
        if (remote && remote.rev > localRev) {
          // Cloud is newer (another device, or a fresh browser) → adopt it.
          ...
          setLocalRev(rev)
          await pushRemote(supabase, userId, local, rev)
        }
        if (cancelled) return
        ready.current = true
  ...
  useEffect(() => {
    ...
    pushTimer.current = setTimeout(async () => {
      try {
        ...
        const rev = Date.now()
        setLocalRev(rev)
        await pushRemote(supabase, userId, progressRef.current, rev)
