    ...
    <View style={styles.wrap} onLayout={(e) => setWidth(e.nativeEvent.layout.width)}>
      ...
      {waiting > 0 && (
        <View style={styles.prompt}>
          {/* A seedling: these kanji have grown enough to bear words you haven't been shown. The
              app's only growth glyph, so it isn't borrowing a meaning another screen has claimed. */}
          <Icon name="seedling" size={12} color={colors.readyInk} />
          {/* The question carries the weight; what follows is the answer to it, in the same
              violet as the tiles the message is about. */}
          <Text style={styles.promptText}>
            <Text style={styles.promptLead}>Ready for more?</Text>{' '}
            {waiting} {waiting === 1 ? 'kanji has' : 'kanji have'} new words waiting.
          </Text>
        </View>
      )}
      {units.length === 0 ? (
        ...
        <>
          {/* The key, directly above the grid it explains and after the filters that change it. */}
          <Legend
            items={TIERS.map(({ tier, label }) => ({
              color: StyleSheet.flatten(styles[tier]).backgroundColor as string,
              label,
            }))}
          />

          {/*
            Rows, not a single wrapping list: the stagger then costs ~18 animated nodes instead of
            one per tile, which is what made the board janky. `cols` is already known from the
            centring maths, so the rows here are exactly the ones flex-wrap would have produced.
          */}
              ...
              <Animated.View key={r} style={[styles.grid, rowAnim[r]]}>
                {row.map((u) => (
                  <Pressable
                    ...
                    accessibilityLabel={
                      stateOf(u) === 'more'
                        ? `${u.form}, ready for more`
                        : `${u.form}, ${stateOf(u)}`
                    }
                    accessibilityHint={
                      stateOf(u) === 'off'
                        ? 'Hold to turn back on'
                        : 'Hold to turn off for practice'
                    }
                    style={[
                      styles.tile,
                      styles[stateOf(u)],
                  ...
                  >
                    <Text style={[styles.form, stateOf(u) === 'off' && styles.formOff]}>
                      {u.form}
                    </Text>
                  </Pressable>
                ))}
...
const makeStyles = (colors: Palette) =>
  StyleSheet.create({
    ...
    prompt: {
      alignSelf: 'center',
      // The glyph leads the sentence on its line, so the two read as one phrase.
      flexDirection: 'row',
      alignItems: 'center',
      gap: 7,
    ...
    // Violet, matching the tiles it is counting — the message and the thing it points at are one.
    // Shrinks rather than pushing the seedling off the row when the count makes the line long.
    promptText: { flexShrink: 1, color: colors.readyInk, fontFamily: fonts.medium, fontSize: 12, textAlign: 'center' },
    promptLead: { fontFamily: fonts.semibold },
    ...
    // One line, five entries: no wrap, and each item shrinks rather than pushing the row over. The
    // dots do the separating, so the row itself is centred rather than spread.
    ...
    legendDot: { color: colors.muted, fontSize: 13, marginHorizontal: 5 },
    swatch: { width: 8, height: 8, borderRadius: 2 },
    legendText: { color: colors.muted, fontFamily: fonts.body, fontSize: 9, flexShrink: 1 },
  })
